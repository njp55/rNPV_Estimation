    ...
    if st.button("🚀 Run rNPV & Monte Carlo Analysis", type="primary", use_container_width=True):
        ...
        for p in phase_data:
            for year in range(int(p["Duration"])):
                ...
                year_labels.append(f"Y{current_year} ({p['Phase']})")
                current_year += 1
        ...
        for year in range(int(commercial_years)):
            ...
            cash_flows.append(r_cf)
            year_labels.append(f"Y{current_year} (Launch)")
            current_year += 1
        ...
        ax.hist(simulated_npvs, bins=50, color='skyblue', edgecolor='black', alpha=0.7)
        
        p10, p50, p90 = np.percentile(simulated_npvs, [10, 50, 90])
        ax.axvline(p10, color='red', linestyle='dashed', linewidth=2, label=f'10th Pctl (Downside): ${p10:,.0f}M')
        ax.axvline(p50, color='green', linestyle='dashed', linewidth=2, label=f'50th Pctl (Median): ${p50:,.0f}M')
        ax.axvline(p90, color='purple', linestyle='dashed', linewidth=2, label=f'90th Pctl (Upside): ${p90:,.0f}M')
        ...
        st.markdown("Download the year-by-year standard risk-adjusted cash flow projection.")
        
        df_export = pd.DataFrame({
            "Year & Phase": year_labels,
